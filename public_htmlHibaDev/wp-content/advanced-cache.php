...
// WP-Optimize advanced-cache.php (written by version: 3.6.0) (homeurl: https://hiba.dev/) (abspath: /home/sites/38a/9/98cbafd97a/public_html/public_htmlHibaDev/) (do not change this line, it is used for correctness checks)
...
$possible_plugin_locations = array(
	...
	'/home/sites/38a/9/98cbafd97a/public_html/public_htmlHibaDev/wp-content/plugins/wp-optimize/cache',
);
...
if (false === $plugin_location) {
	...
	if (strcasecmp('https://hiba.dev/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', '/home/sites/38a/9/98cbafd97a/public_html/public_htmlHibaDev/wp-content/plugins/wp-optimize/cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-hiba.dev.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-hiba.dev.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-hiba.dev.php');
}
